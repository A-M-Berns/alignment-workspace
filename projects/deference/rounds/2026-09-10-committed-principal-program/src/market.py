...
def simulated_outcomes(cfg=eco.Config(), n=20):
    """Twenty simulated occurrences: worlds alternate between the quiet world and the
    early-objection world, and every third occurrence the advisor uses a direct write
    (`covert`).  The activation outcomes under `cfg`."""
    from .fixtures import W0, W1
    outs = []
    for k in range(1, n + 1):
        w = W0 if k % 2 else W1
        adv = eco.ADVISORS["covert" if k % 3 == 0 else "honest"]
        C, _, _ = eco.activation(eco.Frame(w, eco.PRINCIPALS["reading"], cfg), adv)
