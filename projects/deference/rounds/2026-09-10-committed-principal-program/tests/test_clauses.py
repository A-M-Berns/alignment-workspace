...
class TestClause1_PrincipalExclusiveBinding(unittest.TestCase):
    def test_discharged_on_every_certified_world(self):
        for w in (W0, W1):
            ...
            self.assertEqual(pr.holders(log.prefix(rc["event"]), "bind:P"), {(P, eco.PRINCIPAL_KEY)})
    ...
    def test_countermodel_second_key_binds(self):
        """The warrant registry extends `bind:P` to the advisor's key: its commit of the
        mandated program's output is an authenticated answer receipt (`activated` holds)
        and exclusivity fails.  (Changed from the 2026-09-09 fixture: under re-execution a
        delegate can bind only the program's output, so the advisor's own vector no
        longer activates — `TestP1.test_delegate_binds_only_the_program_output`.)"""
        C, cl, log = eco.activation(frame(W_DELEGATE), eco.ADVISORS["delegate_commit"])
        rc = pr.answer_receipt(pr.account_at(log, len(log), 0))
    ...
    def test_countermodel_forged_author(self):
        ...
        principal's key and author field.  (Changed from the 2026-09-09 fixture: under
        re-execution the forger's own vector is void; only the program's output
        activates.  Authenticity now protects *who* bound; *what* is pinned by the
        program — `TestP1.test_forgery_cannot_choose_the_vector`.)"""
        ...
        def forge(V):
            ...
            for e in log:
                if e.kind == COMMIT:
                    forged.append(e.time, P, COMMIT, {"occ": 0, "key": eco.PRINCIPAL_KEY, "V": V,
                                                      "proc": e.payload["proc"]}, warrant=e.warrant)
                else:
                    forged.events.append(replace(e, index=len(forged.events)))
            return forged
        chosen = forge({"a": F(0), "b": F(1)})
        self.assertFalse(pr.activated(pr.account_at(chosen, len(chosen), 0)))
        mandated = forge(pg.evaluate(prog, eco.reason_trace(log), eco.CANDS))
        t = pr.account_at(mandated, len(mandated), 0)
        self.assertTrue(pr.activated(t))
        self.assertEqual(pr.answer_receipt(t)["author"], P)
        self.assertEqual(eco.Frame.V(mandated), {"a": F(1, 2), "b": F(1, 2)})
