...
class I_TreatmentMismatch(unittest.TestCase):
    def test_whole_lease_promise_refuted_by_prefix_execution(self):
        env = investment_env(d=3)
        promised = (ctrl_invest, "lease")
        executed = (ctrl_invest, ("prefix_then", 1, ctrl_work))
        self.assertFalse(valid_test(promised, executed))
        self.assertEqual(block_value(env, lease(ctrl_invest), "base", 6), Q(1, 2))
        self.assertEqual(block_value(env, prefix_then(1, ctrl_work)(ctrl_invest), "base", 6), Q(5, 18))


class J_AlternativeTreatment(unittest.TestCase):
    def test_promise_under_declared_interruption_is_validly_tested_by_interruption(self):
        """A hypothesis promising 5/18 under the treatment 'one step of the plan, then
        work' is exactly kept by that treatment; the whole-block lease is one treatment
        among others, and test validity is treatment matching."""
        env = investment_env(d=3)
