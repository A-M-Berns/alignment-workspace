...
def prefix_allowance(schedule, coef=1):
    """The uniform online subsidy rule (WEIGHTED_BRIA.md §4, PRESSURE_PASS.md §13–14).

    Support `s(k) = ⌊√(S_k / M_k)⌋` and opening subsidy `A(k, i) = (M_k − M_{k−1}) + 1/k`
    for `i ≤ s(k)`, both computed from the prefix `(S_k, M_k)` once `m_k` is revealed at
    the opening of block `k`.  Credited at the opening (`timing="opening"`), it satisfies
    capital adequacy `A_i(K) − m_K → ∞` and `𝒜_K ≤ 2√(S_K M_K) + √S_K (1 + ln K)` with no
    modulus of convergence.  `coef` scales the jump term; the sharp rejection bound
    `ℓ_K < w_K − A_i(K)` (Lean `record_succ_lt_of_rejected_opening`) needs only 1."""
    import math
    cache = {}
...
# --- continuations are the executable object --------------------------------------
#
# A continuation hypothesis emits `(c, e)`: a causal advisor continuation `c` for the
# next block and a claim `e` about its realized block score.  A test is the derived
# event "h was selected and c was executed under the block contract".  Two different
# continuations are two different objects: `prefix_then(j, tail)(q)` — "q for j steps,
# then tail" — is a continuation distinct from `q` itself, and an outcome from it is not
# a test of a claim about `q` (CONTINUATION_HYPOTHESES.md §2).  `lease` is the identity.
...
def valid_test(claimed, executed):
    """Data is evidence about a continuation claim only when generated by executing the
    continuation the claim names."""
    return claimed == executed
