# Deference Between Epistemic Processes: The Tower, the Negative Result, and Two Positive Constructions (v6)

*An integrated note porting the main theorem of Dorst, Levinstein, Salow, Husic & Fitelson, "Deference Done Better" (DDB, 2021) into the logical-induction (LI) framework of Garrabrant et al. (2016), in light of Weatherson's "Deference and Infinite Frames" (2025) — and then carrying the resulting "tower" picture across the gap between **two distinct processes of different computational strength**, where it meets a sharp negative result and **two** matching positive ones.*

> **About this version (v6).** v6 keeps v5's spine and folds in the material developed since (Jun 24–26 2026):
> - a **second positive construction** — *faithful acceleration*, which forces classwise **Value** by having $A$ predict $H$'s own future credences, over an **admissible domain** rather than only the timely fragment (§5.9);
> - the **admissibility analysis** showing that domain *cannot* be a clean syntactic fragment — PA rebuilds the quote-referencing diagonal by Gödel's lemma — so admissibility is a causal/provenance/conditional condition (§5.10);
> - **domain-relative Value**: the positive gives admissible Value (menus from the admissible family), *not* unrestricted Value, because `Total Trust`$_D$ ⟺ `Value`$_D$ is domain-relative (§5.11);
> - the negative result sharpened from *tracking* to the **tower itself**, with the observation that on the quote-referencing diagonal **both** faces (the tower and averaged calibration) die together (§4.8);
> - two scope caveats promoted into the core — the single-round **"one big lie"** limit and the **good-feedback-only** reach (§6.7);
> - an **honest Lean status** rewritten from the statement-level audit `lean-deference/AUDIT.md`: the kernel proves the deference *algebra*, not the *forcing* (§7);
> - a **Vetting Ledger** (Appendix B) listing every load-bearing claim with its status (Proved / Composition / Squeeze-over-named / Stub / prose) and what is *new* or *changed* since v5 — **this document is meant to be read as the working checklist of things to vet.**
>
> A **v5→v6 map** is at the end. The §0–§4 spine and the frozen/$G$ construction (§5.1–§5.8) are inherited from v5 with light edits; what is new is flagged in the ledger. The v5 lineage follows. v4 established the abstract theory — deference between epistemic processes is the **tower property**, and `Value ⟺ Mart` for any observable, coherent, introspective expert — and left the cross-process case (a novice trusting a *distinct, stronger* process, not its own future self) as the open §11 "real prize." v5 keeps v4 as the anchor but **interweaves** the cross-process results worked out since (Anson Berns's project, with Demski: the "Trust Between Logical Inductors" summary, the "Self-Referential Settlement Target" obstruction note, and the "Frozen-Deliberation Deference" construction), together with the human-endorsed framing of Abram's `li-deference.md`. The document is reorganized around a single spine: *deference is the tower; whether the tower can be **forced** is governed by whether the novice can arbitrage against the expert's market; this is free for the future self, impossible-in-general across processes, and forceable across processes exactly on the timely fragment.*
>
...
10. **A second positive construction — faithful acceleration.** Drop from the pointwise tower to the *averaged* inequality and you reach *every admissible sentence* with the realistic **unfrozen** human: an $A$ that predicts $H$'s **own future credences**, held calibrated by Expectation Unbiasedness From Feedback, is forced into bounded $\varepsilon$-violation Total Trust, hence **Value** — over an admissible domain, with no appeal to the prior. (§5.9)
11. **The admissible domain, and domain-relative Value.** The positive reaches all sentences *except* the quote-referencing diagonal $g_n\leftrightarrow(a_n\le\tfrac12)$, where the target hard-settles and **both** faces (the tower and averaged calibration) die together. That domain is **not** a syntactic fragment — PA rebuilds $g_n$ by Gödel's diagonal lemma — so it is a causal/provenance/conditional condition. And since `Total Trust`$_D$ ⟺ `Value`$_D$ is domain-relative, the instrumental conclusion is **admissible Value, not unrestricted Value**. (§5.10–§5.11)
12. **Read "kernel-checked" precisely.** The Lean verifies that the deference *algebra and asymptotics compose correctly, given the LI theorems as named hypotheses.* It does **not** verify the *forcing* — that the no-Dutch-book criterion forces the tower/tracking/calibration — which enters as named hypotheses, for the cross-process forcing results often *equivalent to the conclusion they name*. (§7, `lean-deference/AUDIT.md`)

**Lean (Summary).** Five Lean 4.27.0 + Mathlib modules, all `sorry`-free and audited to `[propext, Classical.choice, Quot.sound]`: **`LeanDeference.lean`** (the tower / `Value ⟺ Mart` core, §1–§3), **`SelfReferentialTarget.lean`** (the obstruction, §4), **`FrozenDeliberation.lean`** (the §3.3 hinge + the §5 frozen suite), **`FaithfulAcceleration.lean`** (the §5.9 credence round-trip), and **`TowerAndAcceleration.lean`** (the §4.8 tower⟹tracking reduction + the two-faces witness). **But "kernel-checked" here means the *algebra composes*, not the forcing** — see point 12 and §7. Each section ends with a **Lean (§N)** note quoting the exact statements proved for it, so the LaTeX and the formalization can be eyeballed side by side. Quoting convention: universe/instance binders (`{W J : Type*}`, `[Fintype W]`, …) are elided as `...`; every semantic hypothesis and the conclusion is verbatim. The LI paper's theorems (the criterion, `loe`, `expprovind`, Non-Dogmatism, per-member convergence) and the LUV/market machinery are the *trusted boundary* (§7) — they enter only as named hypotheses over the real-sequence abstractions of §0.
...
\operatorname{Ind}_\delta(y>t)=\begin{cases}0 & y\le t\\ (y-t)/\delta & t<y\le t+\delta\\ 1 & y>t+\delta,\end{cases}\qquad \operatorname{Ind}_\delta(z):=\operatorname{Ind}_\delta(z>0),\quad \operatorname{Ind}_\delta(y<t):=\operatorname{Ind}_\delta(t-y).
$$
...
### 4.8 The tower itself is dead — and the boundary it shares with the positive result

§4.3's 2a/2b refute *tracking* ($a_n\approx_n Y_n$). The **timely pointwise tower** `Mart$(H\to A)$` — the deference object proper — **reduces** to tracking: read-off ($E^H_n(\ulcorner a_n\urcorner)\approx_n a_n$, $a_n$ being a decided feature of $H$'s world) and self-trust `cee` ($E^H_n(X)\approx_n E^H_n(\ulcorner Y_n\urcorner)$) collapse the two corner-quotes, leaving $a_n\approx_n Y_n$. So the tower **inherits the impossibility**: `Mart$(H\to A)$` is unattainable on every contract family (`anson-notes/no-timely-pointwise-tower.md`; the reduction is `TowerAndAcceleration.tower_imp_tracking`). Quote-referencing families die by 2a (defect $\ge\tfrac12$); quote-free families by 2b (underivability); the two exhaust all families.

The sharper point — which §5.9's positive construction turns on — is that on the quote-referencing diagonal $g_n\leftrightarrow(a_n\le\tfrac12)$, **both faces die together**: not only the tower, but also the *averaged calibration* that the positive construction needs. Whatever $A$ quotes, some legal gate carries a persistent $\ge\tfrac12$ bias between $a_n$ and the realized $\mathbb 1[a_n\le\tfrac12]$, so Expectation Unbiasedness From Feedback is unsatisfiable there (worked out in §5.10). So the negative result does not merely block the strong (equality) face; it carves the exact boundary — the **admissible domain** (§5.11) — outside which *neither* construction reaches. A bounded sequence *can* be pointwise-wrong-yet-averaged-unbiased (the witness `two_faces_distinct`), but only by **decoupling** forecast from realized value, which the coupled diagonal forbids.

---

## 5. Across processes, positively: two constructions (the tower on $G$; faithful acceleration on the admissible domain)

There are **two** positive constructions, complementary trade-offs on the same object $a_n=E^A_n(\ulcorner Y_n\urcorner)$. §5.1–§5.8 give the first — the *sealed-sibling / frozen* target that forces the **per-day tower on the timely fragment $G$** (early *correctness* where questions resolve). §5.9–§5.11 give the second — *faithful acceleration*, which forces **averaged Total Trust → Value over the admissible domain** with the realistic *unfrozen* human (faithful relay of $H$'s own judgment, everywhere admissible). The first construction:
...
### 5.9 The second positive construction: faithful acceleration

The frozen construction buys the per-day tower but pays twice — it must **seal** the target (blindness) and it reaches only $G$. A different trade-off (`faithful-acceleration.md`): keep the **unfrozen, coupled** human, drop from the pointwise tower to the *averaged* inequality, and in exchange reach **every admissible sentence** with no appeal to the prior. The single idea: **defer to the human's *credences*, not the world's *truth*.** Every §4 obstruction is an artifact of forcing $A$ to track *truth*; they dissolve when $A$ tracks where $H$'s own deliberation is heading, because a credence is always-present, continuous, and self-trusted — exactly what the truth target lacks.

**The object.** $A$ forecasts $H$'s own lookahead credence: $a_n := E^A_n(\ulcorner Y_n\urcorner)$, $Y_n := E^H_{f(n)}(X)$ — the *same* $a_n$ as §4.2's mirror, but now $H$ reads $A$ (coupled) and we ask only the averaged face. $E^\ast(X):=a_n$ is coherent (linear $E^A_n$ of linear $E^H_{f(n)}$) and observable (the $H$-reads-$A$ channel), so §1.2's forced **Total Trust ⟺ Value** applies.

**Two forced ingredients**, both free of forward simulation:
- **(I) self-trust** `cee`: $E^H_n(X)\approx_n E^H_n(\ulcorner Y_n\urcorner)$ — $H$ trusts where its own deliberation is heading.
- **(II) calibration** = **Expectation Unbiasedness From Feedback** (Garrabrant Thm 4.8.16) at $B_n=\ulcorner Y_n\urcorner$: $H$ is computable (even reading $A$), so $Y_n$ is determined-via-$\Gamma$, and its realized value enters $A$'s world by *observation* at $\sim f(n)$ — hence for any $\mathcal{C}_A$-generable divergent patient $w_n$, $\;\sum_{n\le N} w_n(Y_n-a_n)\big/\sum_{n\le N} w_n\to0$. **(II) is checked after the fact** (wait → observe → bank), so 2b's cost-circularity never arises and **no power assumption** is needed — this is what lets the unfrozen target work where the §4.2 mirror's *timely* target could not.

**Theorem (faithful acceleration — bounded $\varepsilon$-violation Total Trust, hence classwise Value).** Fix an admissible $X$ and $t,\varepsilon,\delta$; let the violation weight $w_n(X,t,\varepsilon,\delta)=\operatorname{Ind}_\delta(a_n>t)\cdot\operatorname{Ind}_\delta(E^H_n(X)<t-\varepsilon)$ (positive exactly where $A$ flags high while $H$ sits below $t-\varepsilon$). Then $\sum_n w_n<\infty$ — bounded $\varepsilon$-violation Total Trust. As a *one-directional* consequence (not an equivalence — faithful §5), the gate-weighted average of $H$'s own credence over the class $\{a_n>t\}$ has $\liminf_N\ge t-\varepsilon-\delta$ ($\to t$ as $\varepsilon,\delta\to0$); and by the §1.2 witness, bounded $\varepsilon$-violation Total Trust gives classwise **Value** on that class. *Proof (trader):* the **self-trust round-trip** — buy $X$ at $n$, sell at $H$'s own future price $Y_n$, on a sparse schedule with disjoint windows, weighted to fire only where $A$ flags high — banks through day $N$ at least $\varepsilon W_N+\sum_{S,\,n\le N} w_n(Y_n-a_n)$, where $W_N:=\sum_{S,\,n\le N} w_n\to\infty$; ingredient (II) makes the forecast-bias ratio $\sum_{S,\,n\le N} w_n(Y_n-a_n)/W_N\to0$, ingredient (I) makes "sell at $Y_n$" a legitimate cash-out, and one-sidedness supplies the $\varepsilon$: the bank $\to+\infty$. Unbounded value, bounded risk — an exploit $H$ does not admit. The trader core is kernel-checked (`FaithfulAcceleration.lean`; but see §7 — what is checked is the *arithmetic*, with calibration and the criterion named).

**Honest strength.** This is the **averaged** form — the §5.6 / T5 ceiling — *not* the per-day tower (which §4.8 kills off $G$). A day-averaged input (calibration) buys exactly a day-averaged conclusion; the proof never uses pointwise tracking $a_n\approx Y_n$, which is ruled out for the real human. The honest match: averaged in, averaged out. (On $G$ the frozen construction *does* get the per-day tower; the two are complementary — §5.11.)

### 5.10 The admissible domain — and why it cannot be syntactic

The theorem's reach is **not** literally "all sentences" (as `faithful-acceleration.md` §4/§6 still states — that over-claim is corrected here per `faithful-acceleration-scope.md`). The boundary is §4's obstruction reappearing *inside ingredient (II)*. Mutual observability means $H$ reads $a_n$, so $H$'s language can name $a_n$; compare two self-references:

- **price-level liar** $\chi\leftrightarrow(\mathbb{P}^H(\chi)<\tfrac12)$ — through $H$'s *own price*. The market clears it continuously at $\tfrac12$ (Brouwer); $\chi$ is **never decided**; $E^H_{f(n)}(\chi)\approx\tfrac12$, independent of $a_n$. **Benign** — $A$ calibrates to it.
- **quote-referencing diagonal** $g_n\leftrightarrow(a_n\le\tfrac12)$ — through $A$'s *quote, which $H$ reads*. Then "$a_n\le\tfrac12$" is a *decided* fact, $g_n$ hard-decides, and $E^H_{f(n)}(g_n)\to\mathbb 1[a_n\le\tfrac12]$ — a $0/1$ value engineered to be the *opposite* of where $a_n$ sits. **Fatal.**

On $g_n$, **both faces die** (§4.8): the tower by 2a, and calibration too — whatever $A$ quotes, the high-side gate $\operatorname{Ind}_\delta(a_n>\tfrac12)$ (or, if $A$ pins $\tfrac12$, the uniform weight) carries a persistent $\ge\tfrac12$ bias, so (II) is unsatisfiable and **cannot be invoked**. The $\chi$-paradox of 2a reappears inside ingredient (II); calibration breaks exactly where tracking does. So faithful acceleration does not *conquer* the diagonal — it **avoids** it. The credence *existing* (faithful §1's "always present") is necessary but **not sufficient**; it is *continuity* — secured only on the admissible domain — that ingredient (II) needs.

**Why the domain cannot be a syntactic fragment.** One wants "admissible = quote-free PA." Impossible. If the language can represent $A$'s computable quote process — and full PA can, via Gödel numbering — the diagonal lemma rebuilds $g_n$: for the representable $Q_A(x)=$"$A$'s quote on $x$ is $\le\tfrac12$" there is $G$ with $G\leftrightarrow Q_A(\ulcorner G\urcorner)$. Any restriction that (i) keeps full PA arithmetic, (ii) is closed under arithmetized diagonalization, and (iii) excludes the quote diagonal is **inconsistent** — drop one. So admissibility is a **causal / provenance / conditional** condition, not a syntactic one. Three honest framings (none as clean as "all sentences"):

1. **Exogenous task families.** Quantify over a fixed family $X_n$ chosen *independently of $A$'s current quote* (forecast the theorem, choose the policy, evaluate the claim — not "evaluate a sentence built from your quote"). Faithful acceleration is then a theorem *schema about a given family*, not a universal theorem over the language; the price is Value for that family's menus, not all menus.
2. **Separate payoffs from observations.** Two layers $L_{\text{task}}$ (payoff/world claims) and $L_{\text{obs}}$ (the quote ledger/advice). $H$ may use $L_{\text{obs}}$ to *decide whether to defer*, but payoff LUVs must come from $L_{\text{task}}$ — payoffs may not be downstream of the expert's report. The diagonal violates exactly this (it is a deference-punishing payoff, "pays \$1 iff the expert says low"). This is a real provenance/type restriction, not "no quote symbol appears" (PA re-encodes $A$ regardless).
3. **Conditional on calibration.** State the result with the calibration condition as an explicit antecedent: *if the family's feedback satisfies Expectation Unbiasedness From Feedback, then bounded $\varepsilon$-violation Total Trust, hence Value on that family.* This is what `FaithfulAcceleration.lean` actually proves — the calibration hypothesis is the gatekeeper, and "admissible" = "targets for which the antecedent holds."

The cleanest current statement is **(3)** with **(1)/(2)** as the intended models: *for any exogenously-specified target/menu family whose feedback satisfies the weighted-calibration condition, an $A$ forecasting $H$'s future credences is forced into bounded $\varepsilon$-violation Total Trust, hence classwise Value on that family.* That faithful acceleration should be framed as a **task-family theorem, not a universal theorem over a rich arithmetic language**, is the deepest open item of the positive result (§8).

### 5.11 Where Value sits: domain-relative, and the trichotomy

**Value is one rung below the tower, and the negative kills only the rung above it.** The instrumental notion is pinned by §1.2 to the *inequality* face — $\textbf{Value}\Leftrightarrow\textbf{Total Trust}$, forced for any observable coherent expert by the two-option witness, *using no tower*, and *tight*. The tower sits strictly above ($\textbf{Mart}\Rightarrow\textbf{Total Trust}\Leftrightarrow\textbf{Value}$; the reverse squeeze §1.6 fails). So the dead pointwise tower (§4.8) forecloses the tower-route and the per-day grade but **cannot touch Value**, which is weaker than the casualty; and §5.9 installs the Value rung from calibration at **zero loss from Total Trust to Value** (the witness is tight). The instrumental target is reached at exactly the grade of the epistemic one — *classwise*.

**But Value is domain-relative.** The equivalence is $\textbf{Total Trust}_D\Leftrightarrow\textbf{Value}_D$: Total Trust over a domain $D$ matches Value for *menus built from $D$*. It does **not** give "Total Trust on some $X$ ⟹ Value for all menus of all LUVs." If $D$ is all PA LUVs, $D$ contains the quote diagonal and the *left* side already fails (§5.10); the two-option witness makes it exact — for $X=g_n$, Total Trust fails, so the menu $\{g_n,\text{const }s\}$ witnesses unrestricted-Value failure. So the positive supports **admissible Value** (menus from the admissible family), **not** unrestricted Value. Both faces dying together on $g_n$ is the real "fit" between the negative and positive results.

**The trichotomy** (same $a_n$, three trade-offs):

| target | face demanded | reach | verdict |
|---|---|---|---|
| **unfrozen** (coupled $H$) | pointwise equality (tower) | all families | **impossible** (§4.8; 2a/2b exhaust) |
| **unfrozen** (coupled $H$) | averaged inequality (Total Trust → Value) | **admissible domain** | **forced** (§5.9; modulo obl. 3 + the §5.10 framing) |
| **sealed/frozen** sibling | pointwise equality (tower) | timely fragment $G$ | **forced** (§5.1–§5.8) |

To keep the *pointwise equality* you must seal the target and retreat to $G$; to keep the *whole admissible domain* with the realistic unfrozen human you drop to the *averaged* face. On the quote-referencing diagonal — the complement of the admissible domain — **neither** survives. The two positive corners are complementary: *soundness on the checkable fragment* (the AI is right, early) and *faithful acceleration on the admissible domain* (the AI relays your own judgment, early).
...
### 6.7 Two scope caveats the averaged form carries

The faithful-acceleration Value (§5.9) is **averaged**, and two limits of that form belong in the headline, not a footnote (both surfaced by the `deference-trust-lab` red-team):

- **The single-round "one big lie."** Every LI endorsement here — Value included — is a $\approx_w$ *average* over a divergent weighting, and is **structurally silent about any individual high-stakes round**: a treacherous trader can concentrate its whole bounded budget into one decision, and an average-to-zero defect is consistent with a single large spike (the lab's `averaging-hides-spikes`). A genuine single-round guarantee would need a non-asymptotic refinement of LI that does not exist. So "you may act on $A$'s advice" is a *policy-over-a-class* license, never a *this-decision* one.
- **Good-feedback-only reach.** The calibration trader (II) banks only where the feedback target is observed on schedule — the **good-feedback subsequence**. On the unobservable class (flourishing, values, long-horizon ethics — exactly where humans most need to trust an AI), the calibration hypothesis is *vacuous* and the construction gives nothing for free. The forcing's true support is good-feedback ⊂ decidable ⊂ outside the undecidable danger zone (§5.8) — which is also where deference is least needed.

These do not weaken faithful acceleration; they state precisely what "averaged Value over the admissible domain" does and does not license.
...
**Five** Lean 4.27.0 + Mathlib modules and a Python check, all `sorry`-free and audited to `#print axioms = [propext, Classical.choice, Quot.sound]` (≈129 results in total). **The label must be read precisely** — a statement-level adversarial audit (`lean-deference/AUDIT.md`, 2026-06-26) finds one fault line that is the whole story:

> The Lean proves the **implications** of the deference theory. It does not prove the **antecedents** — the *forcing*.

What is genuinely established is the **conditional skeleton**: `tower ⟹ Value`; `Value ⟺ Total Trust` (from linearity, both arrows); the finite-frame algebra (`decomposition`, `witness_identity`, the softmax bound `softmax_lower_bound`, the immodesty collapse `CM_implies_immodest`); the obstruction arithmetic (`no_exact_quote`'s ½-gap, `cost_circularity`'s regress); the `two_faces_distinct` witness; and the `≈ₙ`/`≳ₙ` calculus that glues them. What is **not** established — what enters as named hypotheses, *and for the cross-process forcing results as named hypotheses essentially equivalent to the conclusion* — is that the no-Dutch-book criterion **forces** the tower / tracking / calibration. **The market and traders are entirely unmodeled**, so every "the criterion forbids the exploit" is a named hypothesis or a trivial arithmetic stub. In particular: T1 `faithful_tracking` and T3 `conditional_tower` are *squeezes over hypotheses that are the conclusion restated*; `underdetermination_off_G` proves "two points in $(0,1)$" under a model-theoretic name; the §4.4 dichotomy `predictable_imp_uninfluenced` is propositional, never instantiated from the real 2a/2b; the §3.3 complexity content is abstract cost sequences. None of this is hidden by `#print axioms` (which certifies only that each proof inhabits its stated type). The calibrated reading: **the deference algebra and asymptotics compose correctly given the LI literature and a set of modeling identifications — and for the forcing results the substantive claim (criterion ⇒ tower) is among those identifications, not the proved content.** The per-claim status is Appendix B. The five modules:
...
**`lean-deference/FaithfulAcceleration.lean` (the §5.9 credence round-trip, 16 decls):** the trader chain `round_profit_ge → profit_partial_sum_ge → profit_diverges → violation_not_persistent → soft_total_trust` — a genuine arithmetic/asymptotic **composition** (per-round profit bound; summation; "calibration + divergent weight ⇒ banked value diverges"; "criterion ⇒ weight bounded"). The doubly-soft weight is now **constructed** — `softInd`/`dsWeight`, with `dsWeight_continuous` proving the joint continuity a hard indicator violates and *discharging* the support hypotheses `hone`/`hmis` from the construction (this closed one earlier type-(c) leak). Still **named** (the forcing): calibration `hbias` (= Expectation Unbiasedness From Feedback) and the criterion `hbdd`. `profit_diverges_nonvacuous` guards vacuity; Total Trust ⟹ Value reuses `LeanDeference.value_iff_totalTrust`.

**`lean-deference/TowerAndAcceleration.lean` (the §4.8 reduction + the two-faces witness, 14 decls):** `tower_imp_tracking` — the pure `≈ₙ` reduction of the timely tower to tracking via the three corner-quote collapses (taken as hypotheses); `pointwise_tower_fails` composes it with the named `tracking_fails`. `witness_partial_sum`/`witness_gap`/`witness_tracking_fails`/`two_faces_distinct` — a genuine, non-degenerate witness ($a_n\equiv\tfrac12$, $Y_n$ alternating) that is pointwise-wrong by ½ yet aggregate-unbiased, formalizing the "per-day dead, averaged alive" distinction (§4.8, §5.11). The corner-quote collapses and `tracking_fails` are the named inputs.
...
- **Faithful acceleration's admissible domain (§5.10) — the deepest open item of the positive result.** There is no broad PA-syntactic restriction that keeps full arithmetic yet excludes the quote-referencing diagonal (Gödel rebuilds it). So the positive must be a **task-family theorem** under a causal/provenance/calibration condition, not a universal theorem over the language. Open: pin the right admissibility condition (exogenous-family / payoff-observation separation / calibration-as-antecedent) and prove a clean construction always *stays* admissible (faithful §8 obl. 3, "no hidden hard settlement").
- **Cross-agent LUV-Total-Trust between distinct inductors (D3) — the common open problem.** Every constructive thread in the `deference-trust-lab` (the Eisenstat merge, legitimacy-as-LUV-Total-Trust, weak-endorsement) **reduces to** this one object, and it is the part nobody has discharged (it is v4 §11 / the §3.4 governing question). The reductions are genuine; the discharge is open. The lab's `merging-inductors` reframes Eisenstat's fast-student/slow-teacher as "§5.9's expert $B_n=E^A_n(\ulcorner\mathbb{P}^H_{f(n)}\urcorner)$," with three named sub-gaps — is $B$ a logical inductor; the $\mathbb{P}^A\!\to\!\mathbb{P}^H$ standpoint shift; $B$-observability — *discharged in zero cases*. *(Attribution caveat, 2026-08-10: the lab's and this document's construals do not match the information structure Sam Eisenstat intended — AI reads human beliefs immediately, humans see AI beliefs only at a delay — so neither the §4 negatives nor this open-problem framing is a verdict on his conjecture; see `wiki/eisenstat-conjecture-attribution.md`.)*
- **A single-round (non-asymptotic) refinement of LI (§6.7).** Every guarantee here is a $\approx_w$ average; a "this decision is safe" guarantee would need a non-asymptotic LI that does not exist. This is the gap between policy-over-a-class safety and per-decision safety, and it is structural.
- **Model a minimal market in Lean so "criterion ⇒ forcing" becomes a theorem (§7, `AUDIT.md`).** The cross-process *forcing* is currently named, not proved (T1/T3 are squeezes over hypotheses equivalent to their conclusions; the market is unmodeled). A single abstract "legal strategy" type with a continuity field and a no-persistent-profit axiom would convert the §3.1/§3.3 gaps from named-hypothesis to theorem — the only route to real assurance for the forcing claims.
- **Exploratory directions (`deference-trust-lab` — mostly conjecture/ideation; cite with care).** The **asymmetry of the deferral $f$** (ask $A$ about $H$'s *future*) is load-bearing — it breaks the regress that would otherwise collapse two mutually-trusting modest agents into one. **Aumann-agreement fails under modesty** (persistent human–AI disagreement is not by itself evidence of misalignment; built finitely as the lab's `aumann-modesty`, a single-agent illustration of Geanakoplos). The legitimacy/corrigibility, updateless-deference, and unbounded-embedded-agency threads are ideated and red-teamed but **not** verified theorems — every lab Lean file is a finite δ=0 shadow with the LI content entered as hypotheses, and `trust-laundering` is a refuted shadow (do not cite its numbers).
...
| area | v5 | v6 |
|---|---|---|
| negative | 2a/2b kill *tracking* (§4.3) | **+ §4.8**: the *tower itself* is dead (the reduction); both faces die on the quote-referencing diagonal |
| positive | one construction (frozen, tower on $G$) | **+ §5.9** faithful acceleration: averaged Value, *unfrozen* human, admissible domain |
| scope | "all sentences" (`faithful-acceleration.md`) | **§5.10 admissible domain**; *not* syntactic (Gödel rebuilds the diagonal); task-family / provenance / conditional framings |
| Value | `Value ⟺ Mart` / Total Trust | **§5.11 domain-relative**: `Value`$_D$ ⟺ `Total Trust`$_D$ — admissible Value, not unrestricted |
| caveats | — | **§6.7** single-round "one big lie"; good-feedback-only reach |
| Lean | "67 results kernel-checked" | **§7 honest**: the algebra is proved, the *forcing* is named (per `lean-deference/AUDIT.md`); 5 modules, ≈129 results |
| vetting | §7 + §8 (consolidated) | **+ Appendix B** vetting ledger: per-claim status + new/changed since v5 |
...
- **"The Frozen-Deliberation Deference Construction"** — `anson-notes/frozen-deliberation-deference-v6.md`. The sealed-sibling forcing construction, T1–T7, Target-Soundness and the seam (§5.1–§5.8), and the plain-language alignment reading (§6). Kernel-checked in `lean-deference/FrozenDeliberation.lean`.
- Claude Opus 4.8 (with Demski), **"Faithful Acceleration"** — `faithful-acceleration.md`. The §5.9 credence round-trip and classwise Value. Trader core kernel-checked in `lean-deference/FaithfulAcceleration.lean`. *Its "all sentences" scope is corrected here to the admissible domain (§5.10).*
- Claude Opus 4.8, **"No Timely Pointwise Tower"** — `anson-notes/no-timely-pointwise-tower.md`. The negative result sharpened from tracking to the tower itself (§4.8); the tower⟹tracking reduction is kernel-checked in `lean-deference/TowerAndAcceleration.lean`.
- Claude / Demski (with a Codex second opinion), **"Does faithful acceleration reach all sentences?"** — `faithful-acceleration-scope.md`. The admissible-domain analysis (§5.10): $\chi$ vs. $g_n$, both faces dying, and the verdict that faithful over-claims. The Codex review adds the **domain-relativity of Value** (§5.11) and the **PA/Gödel impossibility** of a syntactic admissible fragment with its three framings (§5.10).
- Claude Opus 4.8, **"The Pointwise Tower and Faithful Acceleration"** — `pointwise-tower-and-faithful-acceleration.md`. The synthesis explainer; §5.11's "where Value sits" framing originates there.
- Claude Opus 4.8, **"Lean Audit"** — `lean-deference/AUDIT.md`. The statement-level adversarial audit (algebra proved, forcing named) driving §7 and Appendix B.
- **`deference-trust-lab/`** — the exploration lab (updateless deference, merging inductors, legitimacy/corrigibility, weak-endorsement, unbounded embedded agency), with red-team and Lean-verify reports. Source of §6.7's caveats and §8's exploratory directions; mostly conjecture/ideation (see its `report/CRITIQUE.md`).
...
## Appendix A — Lean per-theorem ledger

For the declaration-by-declaration status of all five Lean modules (kind + hypothesis provenance, with the "engineered to typecheck" cases called out), see **`lean-deference/AUDIT.md` Appendix A**. Appendix B below is the *claim-level* vetting list (prose + Lean together), keyed to this document's sections.

## Appendix B — Vetting ledger

*This is the working checklist. One row per load-bearing claim. **Status:** `P` proved/kernel-checked · `C` composition (chains named LI facts; real work, named inputs) · `S` squeeze (one step over hypotheses ≈ the conclusion — weak check) · `T` trivial stub (stands for an unmodeled argument) · `Pr` prose (argued, not in Lean) · `LI` taken from the LI paper as a black box · `O` open/conjecture. **Δ since v5:** `=` unchanged · `+` new in v6 · `~` changed in v6. Read with §7's fault line in mind: the kernel proves the algebra, not the forcing. Highest-priority to vet: the `+`/`~` rows, and every `S`/`Pr`/`O` on a forcing or admissibility claim.*

**Abstract theory (§1–§2) — the most solid tier.**

| claim | status | Δ | where |
|---|---|---|---|
| Deference is the tower; `Value ⟺ Mart` | P/C | = | §1.1 |
| `Value ⟺ Total Trust` (two-option witness, both arrows) | P | = | §1.2 |
| `Total Trust ⟹ tower` squeeze fails (amplifier obstruction) | P (amp); squeeze itself | = | §1.6 |
| coherent state cheap vs. frame dear | P | = | §2.1 |
| modest+coherent ⇒ infinite frame; soft⇒hard collapse | P (hard tail `CM_implies_immodest`); **soft⇒hard step `Pr`** | = | §2.2 |

**Forcing & the complexity gap (§3).**

| claim | status | Δ | where |
|---|---|---|---|
| a Value gap is pure arbitrage; self-case free | P (arithmetic); forcing `LI`/`T` | = | §3.1–§3.2 |
| complexity-gap hinge (blind cost vs. cost-circularity) | P (abstract sequences); complexity content **interpretive** | = | §3.3 |

**Negative (§4).**

| claim | status | Δ | where |
|---|---|---|---|
| No-Forced-Trust on undecidables | Pr (meta) | = | §4.1 |
| 2a anti-inductive ½-gap (`no_exact_quote`); tracking fails | P + C (named LI-PI) | = | §4.3 |
| 2b cost-circularity regress | P; `hcost` soft joint `LI`/`Pr` | = | §4.3 |
| dichotomy "predictable iff uninfluenced" | **propositional, never instantiated** | = | §4.4 |
| **the *tower itself* is dead** (tower⟹tracking reduction) | P (`tower_imp_tracking`) + C (named `tracking_fails`) | **+** | §4.8 |
| **both faces die on the quote-referencing diagonal** | Pr (the calibration-fails gate argument) | **+** | §4.8, §5.10 |

**Positive — frozen / $G$ (§5.1–§5.8).** *Forcing suite: per §7 the headline forcing is largely named.*

| claim | status | Δ | where |
|---|---|---|---|
| T1 faithful tracking (`a≈Y` forced) | **S** (hyps ≡ conclusion) | = | §5.4 |
| T2 earned meta-trust | S/C | = | §5.4 |
| T3 conditional tower on $G$ | **S** | = | §5.5 |
| T4 Value on $G$ | C (but tower steps = T3) | = | §5.5 |
| T5 pointwise object-level false (= `tracking_fails`) | C | = | §5.6 |
| T6/T7 calibration & limit-agreement on $G$ | S / triangle | = | §5.5–§5.6 |
| T7 underdetermination off $G$ | **T** (two points in (0,1); oversold name) | = | §5.6 |
| safety/silence (forcing co-extensive w/ settlement) | T (`0·ε=0`) / Archimedean | = | §5.8 |
| resource-bounded existence theorem for $A$ | **O** (prose) | = | §5.7, §8 |

**Positive — faithful acceleration (§5.9–§5.11). All new.**

| claim | status | Δ | where |
|---|---|---|---|
| bounded $\varepsilon$-violation Total Trust → Value (credence round-trip) | C (arithmetic chain); calibration `hbias` + criterion `hbdd` **named** | **+** | §5.9 |
| doubly-soft weight is a legal trade-weight | P (joint continuity); "continuous ⟹ legal $\mathcal{C}_H$-feature" still `Pr` | **+** | §5.9 |
| `two_faces_distinct` (pointwise-wrong, averaged-unbiased) | P | **+** | §5.11 |
| **admissible domain ≠ syntactic fragment** (PA/Gödel rebuilds $g_n$) | Pr (sound diagonal-lemma argument) | **+** | §5.10 |
| calibration fails on $g_n$ (the gate argument) | Pr | **+** | §5.10 |
| which admissibility framing (exogenous / provenance / conditional) | **O** | **+** | §5.10, §8 |
| **Value is domain-relative** (`Value`$_D$ ⟺ `TT`$_D$; admissible Value only) | P (equivalence); domain-relativity interpretation | **~** | §5.11 |
| the trichotomy (three trade-offs on $a_n$) | synthesis (rests on the above) | **+** | §5.11 |

**Caveats & meaning (§6).**

| claim | status | Δ | where |
|---|---|---|---|
| manipulation boundary (faithful ≠ sound off $G$; whispering earring) | Pr (rests on T1/T7) | = | §6.3 |
| single-round "one big lie" (averaged ⇏ per-round) | Pr (lab `averaging-hides-spikes` finite witness) | **+** | §6.7 |
| good-feedback-only reach (vacuous on the unobservable class) | Pr | **+** | §6.7 |
| corrigibility / legitimacy program | O (research program) | = | §6.4–§6.5 |

**Open frontier (§8).** D3 cross-agent LUV-Total-Trust (the common reduction; discharged in zero cases); the admissibility characterization; a non-asymptotic LI for single-round safety; a modeled market so "criterion ⇒ forcing" is a Lean theorem; the lab's exploratory threads (all `O`). All **new or restated** in v6.

*One-line status of the whole document: the **abstract theory** (§1–§2) and the **obstruction arithmetic** (§4.3, §4.8) are solid; the **forcing** (frozen §5 and faithful-acceleration §5.9) is real mathematics whose load-bearing step — that the criterion forces the inequality — is **named, not proved** (§7); the **admissibility/scope** (§5.10) and the **caveats** (§6.7) are sound prose not yet formalized; and the **extensions** (§8) are open.*
