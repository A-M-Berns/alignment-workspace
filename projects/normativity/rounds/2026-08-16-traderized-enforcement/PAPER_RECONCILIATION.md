...
**Model B**, which the paper intends: the constraint `K_t` lives in **price**
space, the credences it admits are `C_t = π_t⁻¹(K_t)` where
`π_t(μ) = Σ_ω μ(ω)·ω` is the pricing map, and

    Ω_t^live  =  { ω : ∃ μ ∈ C_t, μ(ω) > 0 } .

A world is live when *some* admitted credence gives it positive mass. It does not
have to be admissible as a point mass.
...
**Forward.** If `ω ∈ PC(D_t)` then `π_t(δ_ω) ∈ K_t^D` by definition of `K_t^D` as
the image of the whole simplex over `PC(D_t)`, so `δ_ω` is a compatible credence
and `ω` is live.

**Reverse.** If `ω` is live, some `μ ∈ C_t` has `μ(ω) > 0`. Writing `K_t^D` as the
image of `Δ(PC(D_t))` is what makes this immediate: `π_t(μ) ∈ K_t^D` means
`π_t(μ) = π_t(ν)` for some `ν` supported on `PC(D_t)`, and on a fragment where the
pricing map separates the worlds this forces `μ = ν`, hence `ω ∈ PC(D_t)`. Where
the pricing map does *not* separate worlds the conclusion needs the separating
form of the region, which is why the formulation `K_t^D = π_t(Δ(PC(D_t)))` is
preferable to any presentation by rows: the reverse direction is immediate from it
and is not immediate from a row system.
...
the same set, and both directions are exercised separately
(`test_semantics.DeductiveRecoveryUnderSupport`). `test-supported` on the
displayed fragment; **derived** in general subject to the separation caveat.
...
6. **What lets admissible credal guarantees control worldwise live-world
   losses** is a bridge hypothesis, because the enforcement inequality bounds
   expectations and the criterion is worldwise. Two sufficient routes: the
   deficit bound `L_t(ω) ≤ ∑_j β_j g_j d_j(ω)`, which needs no support
   hypothesis; and quantitative support coverage `θ_t(ω) ≥ θ`, which gives
   `E_t(ω) ≥ −(1−θ)U_t/θ` with `U_t` the position's cube maximum gain. Neither is
   shown necessary.
7. **Ordinary deduction falls out** by taking `K_t^D = π_t(Δ(PC(D_t)))`,
   whereupon `Ω_t^live = PC(D_t)` in both directions and the generalized
   criterion is `LIC_D`.

**All seven are now precise.** What remains open is not a sentence but two
ordinary questions: whether either bridge hypothesis is necessary, and what
governs a source setting a world's support capacity to zero outright — which is
the narrow residue of the withdrawn laundering worry, and is settlement-shaped
