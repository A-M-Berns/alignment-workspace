...
| 27c | settlement monotonicity makes the exclusion depth non-increasing for a fixed endorsement under irreversible settlement — **helpful, neither necessary nor sufficient** | `NL-SI-C4`, `NL-SI-P1` | derived | `test_normative.SettlementDrivesTheGapDown` | yes | no |
...
| 27r | one **global** affine endorsement `c = ½B + ¼C + Σ_j 2^-(j+2)A_j` at `r = 3/4` gives closed forms `m_t = 3/4 − 2^-(t+2)` and `D_t = 2^-(t+2)`: never vindicated at any finite date, positive core minimum throughout, cost series `9/8` in closed form | affine rows are in scope by `NL-SI-A2`/`A5`, though every displayed source instance is sentence-shaped | derived (closed forms), witness (finite stages) | `test_normative.StaticsGenerateAForeverUnvindicatedTrajectory` | yes | no |
| 27t | **certificate substitution**: a verified certificate for `p ≥ 0` funded enforcement of `p ≥ ½` for nothing while the emitted position really lost at a live world | pre-repair API | witness | `test_outflow.CertificateSubstitution` | yes | no |
| 27u | a verified deficit certificate binds to the exact date, support, row presentation and live-world set — all four, the fourth having been unchecked and separately exploitable | — | derived | `test_outflow.CertificateSubstitution` | yes | no |
| 27u' | **row permutation invariance**: position, weighted square, liability and certified aggregate are all sums over rows at uniform intensity, so a permutation moves none of them; the presentation key canonicalizes order and keeps multiplicity | — | derived | `test_outflow.RowPermutationIsInvariant` | yes | no |
| 27z | the motivating trajectories run end to end through the public `compile_safe_force`: certified `D_t` equals the closed form and the charge equals `q_t` | affine stages and the sentence-shaped vindication | witness | `test_normative.MotivatingTrajectoriesRunThroughTheSafeAPI` | yes | no |
| 27v | only a verified, request-bound certificate produces safety-certified force; a `LiveDeficitClaim` prices a request and cannot certify one, and `verified` cannot be set by the ordinary constructor | — | derived | same | yes | no |
| 27w | relaxation never strengthens an affordable request | — | derived | `test_outflow.RelaxOnlyLoosens` | yes | no |
| 27x | endorsement budgets are **caps**, not ring-fenced reserves: `Σ_e B_e ≤ B` and `spent_e ≤ B_e`, but unallocated charges may still spend promised capital | — | derived | `test_outflow.Subaccounts` | yes | no |
| 27y | an automatically satisfied region makes no tolerance meaningful; the two concepts no longer share one boolean | — | derived | `test_outflow.MeaningfulForceIsScaleRelative` | yes | no |
