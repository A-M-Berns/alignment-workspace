...
from outflow import LiveDeficitCertificate, OutflowAccount
...
class StaticsGenerateAForeverUnvindicatedTrajectory(unittest.TestCase):
    """Gradual depth closure, generated by the statics rather than stipulated.

    The abstract fixture in `test_outflow` sets `D_t = 2^-t` by hand; it shows
    the force mechanism admits such a trajectory, not that the motivating
    normative statics produce one. This one is produced.

    The endorsement is a genuine affine functional of priced sentences — not an
    indicator — and its demand sits exactly at the value the settled record
    *approaches*: settlement establishes `A_1, A_2, ...` in turn, each raising
    the worst surviving delivery halfway to `r`, while the growing fragment keeps
    a world below it alive. So the endorsement is **never** vindicated at any
    date, its exclusion depth is positive at every date, and the depth halves.

    That is the shape the safety account can fund indefinitely, and the reason it
    is worth having: the record never settles the question, and the demand is
    still enforceable forever.
    """
    ...
    def test_it_is_never_vindicated(self):
        for t in range(7):
            _, _, c = self.stage(t)
            self.assertLess(min(c), self.rhs)
    ...
    def test_the_depth_halves(self):
        depths = [max(ZERO, self.rhs - min(self.stage(t)[2])) for t in range(7)]
        self.assertEqual(depths[0], F(1, 4))
        for a, b in zip(depths, depths[1:]):
            self.assertEqual(b, a / 2)
            self.assertGreater(b, ZERO)

    def test_the_account_funds_it_within_a_finite_bound(self):
        account = OutflowAccount(F(2))
        for t in range(7):
            _, worlds, c = self.stage(t)
            depth = max(ZERO, self.rhs - min(c))
            account.spend(F(1, 8), F(1), F(1, 2),
                          LiveDeficitCertificate.asserted(
                              t, depth, "worst live delivery of the endorsement"),
                          "e")
        self.assertLess(account.spent, F(2))
