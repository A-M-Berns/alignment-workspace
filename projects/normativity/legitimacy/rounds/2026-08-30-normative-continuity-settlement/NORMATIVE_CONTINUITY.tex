...
\author{}
\date{Revision 2 (mathematical settlement) --- 30 August 2026}
...
$g$. $S=\varnothing$ is a terminal resolution. $\Resolve_n$ is sufficient on its
own: a resolution is not additionally gated by $\Permit_n$. A represented rule
may make $\kappa_q$'s acceptance consult the standing state or a permission
judgment, but that is content of the semantics, not a second structural gate;
no theorem below reads such a gate, so the interface is kept at the weaker
primitive (superseding the frozen kernel's entry A21). If batch $e_n$ actually
records such an accepted resolution, write
\[
 q\res{g}{n}S,
 \qquad
...
All eight judgments read the strict prefix uniformly. This supersedes the
frozen Legitimate Evolution convention (its entry A34), under which $\Due$ read
descriptive material from the current event while normative standing was read
from the pre-state: a record written in $e_n$ here first affects $\Due$ at
$n+1$. Nothing is lost but one position, and the descriptive/normative split
...
grounds. A record that neither adds nor removes standing is a no-op and its
grounds are unconstrained; the frozen kernel's S1 constrained the grounds of
every accepted edit, and the difference has no structural content, because only
a change creates a node of an authorization tree.
\end{requirement}
...
Let the \emph{admitted} occurrences be those that ever gained standing,
\[
 \Adm_n:=G\cup\bigcup_{j<n}L_j^+ ,
\]
so $L_n\subseteq\Adm_n$ and a rule removed from standing stays admitted.

\begin{theorem}[Grounded Replay]\label{thm:replay}
Assume Requirement~\ref{req:standing-change}. For every $\lambda\in\Adm_n$,
...
rule entered $L$, and positions strictly decrease along every branch. In
particular this holds for every $\lambda\in L_n$.
\end{theorem}

\begin{proof}
Induct on $n$. $\Adm_0=G$. A rule in $\Adm_{n+1}\setminus\Adm_n$ entered
standing in $e_n$ through a change whose grounds are standing authorizers at
$H_n$, hence in $\Adm_n$ with trees by the induction hypothesis; hang those
trees under the new node at position $n$. Freshness of $L_n^+$ makes ``the
change through which the rule entered'' unique; the existence of the tree does
not use it.
\end{proof}

The theorem is stated over admitted occurrences because it is a historical
statement: it is exactly ``ancestry, not permanence.'' The standing form is
the corollary its consumer (Requirement~\ref{req:anchor}) needs.
...
disappear. Because $\ResSet_n\subseteq O_n$ and $\Born_n\cap O_n=\varnothing$,
an issue cannot be opened and resolved in the same batch: every fresh issue is
outstanding for at least one position. This deliberately supersedes the frozen
kernel's entry A17. The reason is timing discipline, not content: every
judgment about $q$ reads a prefix at which $q$ is outstanding, so
$s_n(q)$, $\Pre_n(q)$ and $\Ready_n(q)$ are defined whenever $\Resolve_n(q;\cdot)$
is consulted; allowing a birth-batch resolution would need a separate
birth-state semantics, and would let Requirement~\ref{req:due} be met by an
issue that is never outstanding at any prefix.
\end{requirement}
...
Freshness of successors deliberately supersedes the frozen kernel's entries A5
and A11, which allowed a resolution to carry into an issue already outstanding.
The mathematical reason is prefix-determinacy. With a static successor relation
into a preexisting $q'$, $m\preceq q'$ would hold at every prefix, so
$\Live_k(m)$ and $o_k(m)$ for $k<n$ would change when $e_n$ is written:
opportunity would not be a function of the history up to $k$. Time-stamped
successor edges repair this at the cost of time-relative ancestry, and buy
nothing this theory needs: relating an outstanding issue $q$ to an existing
$q'$ is expressed by a prerequisite of $q$ routed to $q'$, which keeps $m$'s
identity, puts $q'$ in $\Reach_n(m)$, and so credits $q'$'s work to $m$
(Section~\ref{sec:work}). Succession is reserved for diachronic continuation of
an entitlement into a fresh episode.
\end{requirement}
...
$\Met$ is a primitive judgment on the condition $\chi_d$. In particular a
route's terminal resolution does not by itself satisfy the prerequisite it was
a route for; whether it does is what the current rules say about $\chi_d$. The
earlier Answerable Process page had instead a structural clause ``met iff some
alternative was resolved terminally'' and no counterpart of
Requirement~\ref{req:ready}; both are deliberate changes.
...
\begin{lemma}[matter bookkeeping]\label{lem:matter-bookkeeping}
$M_n\subseteq M_{n+1}$, and every $m\in M_n$ is an issue opened at some
position $j<n$. Matterhood is never retroactive: $m\notin M_k$ for
$k\le\beta(m)-1$.
\end{lemma}

\begin{proof}
Nothing is removed. A root added after $e_n$ lies in $\Born_n$; a designated
issue lies in $O_n\cup\Born_n$, hence in $\Born_j$ for some $j\le n$. The last
claim is the definition of $\beta$.
\end{proof}

These two facts are all that the theorems of Sections~\ref{sec:work}--\ref{sec:nsa}
use about $M_n$; they are stated so that the results hold for any assignment of
matters with these properties, of which the construction above is one.
...
available work; the second is a unit total service budget. The budget is on matters, including a designated
matter and the matter of its ancestor, which overlap; the earlier Answerable
Process page put the budget on issues and credited matters by a maximum. The
matter-grain budget is stricter under overlap and is satisfiable
(Lemma~\ref{lem:nonstarve-feasible}); no theorem depends on the choice. Here
attention is a service quantity, not a restriction on which structural records
may be written.
...
Fix any injection $\iota:M_\infty\to\mathbb N$ that is determined at birth---for
instance birth order $\beta$, with ties within one batch broken by record
order---and give $m$ the fixed share $w_m=2^{-\iota(m)-1}$. Set
\[
 a_n(m):=w_m\,o_n(m).
\]
Then $0\le a_n(m)\le o_n(m)$; $\sum_{m\in M_n}a_n(m)\le\sum_{k<K}2^{-k-1}<1$
for every finite $M_n$, however many matters appear over time and however they
overlap; and $A_N(m)=w_m\Omega_N(m)$, so every matter satisfies non-starvation
...
No fixed prerequisite remains a no-route wait of a matter forever: for every
matter $m$, prerequisite occurrence $d$, and position $N_0$,
\[
 \exists n\ge N_0:\ d\notin\NoRoute_n(m).
\]
Equivalently, since $d\in\NoRoute_k(m)$ requires $\neg\Met_k(d)$,
\[
 \bigl(\forall n\ge N_0:\ d\in\NoRoute_n(m)\bigr)
 \quad\Longrightarrow\quad
 \exists k\ge N_0:\ \Met_k(d),
\]
which is the form that names a mechanism. The first form is the primitive: it
is exactly the negation of the Persistent-Wait conclusion, and it abstracts
over why the wait clears---the prerequisite may be satisfied, withdrawn, or
cease to be reachable. Coverage of outside inputs and a discipline for removing
obsolete internal prerequisites are possible future sufficient conditions; the
structural theory assumes only this statement and does not classify waits as
internal or external.
\end{assumption}
...
which wait responsiveness denies.
\end{proof}
...
The route-following construction and
Theorems~\ref{thm:persistent-wait}--\ref{thm:nsa} are the newer part of the
synthesis. The no-rewiring rule (Requirement~\ref{req:no-rewire}) descends from
the Answerable Process page's dependency discipline, which permitted a
dependency record only on a frontier issue or a fresh one; that rule implies
Requirement~\ref{req:no-rewire}, which is a deliberate weakening---it also
allows a prerequisite on a non-frontier issue when every matter reaching it has
other work---and is the weakest gate under which the Persistent-Wait proof
goes through. The ownership-only gate (``every matter for which $q$ is a
descendant has work'') is refuted by the cross-matter prerequisite-rotation
counterexample. The notation $\Work_n(m)$ is the same mathematical object
...
This revision settles the modeling choices left open by the concordance of the
\textsc{agent-consolidated} checkpoint and records them where they apply:
successor freshness (Requirement~\ref{req:fresh-successors}), no same-batch
opening and resolution (Requirement~\ref{req:resolution-continuity}), uniform
strict-prefix timing (acceptance convention), resolution not gated by
$\Permit$ (Section~\ref{sec:semantics}), grounds constrained only for changing
standing records (Requirement~\ref{req:standing-change}), Grounded Replay over
admitted occurrences (Theorem~\ref{thm:replay}), $\Met$ as a primitive and
Requirement~\ref{req:ready} (Section~\ref{sec:matters}), the matter-grain
attention budget and the exact positive-share witness
(Lemma~\ref{lem:nonstarve-feasible}), wait responsiveness in its primitive form
(Section~\ref{sec:nsa}), the matter-bookkeeping lemma
(Lemma~\ref{lem:matter-bookkeeping}), the provenance of
Requirement~\ref{req:no-rewire} above, and the removal of an unused macro. No
definition used by a theorem changed.

\paragraph{The theory and the theorems.}
Normatively continuous evolution is \emph{defined} by
Requirements~\ref{req:standing-change}--\ref{req:no-rewire} together with the
compatibility assumption. The derived theorems each use a proper subset, and
the document does not say otherwise anywhere:
\begin{center}\small
\begin{tabular}{@{}p{0.33\textwidth}p{0.17\textwidth}p{0.4\textwidth}@{}}
\toprule
\textbf{condition} & \textbf{part of the theory} & \textbf{used by} \\
\midrule
Req.~\ref{req:standing-change} (authority changes) & yes & Grounded Replay \\
Req.~\ref{req:anchor} (standing at opening) & yes & the bridge from Grounded Replay to issues only \\
Req.~\ref{req:due} (new demands become issues), compatibility & yes & no theorem \\
Req.~\ref{req:resolution-continuity}, \ref{req:fresh-successors} & yes & Lemma~\ref{lem:matter-continuity}, Lemma~\ref{lem:route-extinction}, Persistent-Wait \\
Req.~\ref{req:state} (state continuity) & yes & no theorem \\
Req.~\ref{req:pre-continuity}--\ref{req:ready} & yes & Lemma~\ref{lem:route-extinction} (\ref{req:pre-refs}), Persistent-Wait \\
Req.~\ref{req:nonstarvation} (non-starvation) & yes & No Structural Abandonment \\
Req.~\ref{req:no-rewire} (reach gate) & yes & Persistent-Wait \\
finite batches; matter bookkeeping (Lemma~\ref{lem:matter-bookkeeping}) & bookkeeping & Persistent-Wait \\
...
\end{tabular}
\end{center}
Persistent Opportunity is Persistent-Wait plus wait responsiveness; No
Structural Abandonment is Persistent Opportunity plus
Requirement~\ref{req:nonstarvation}. Lemma~\ref{lem:root-matter} is used by no
theorem.
...
\paragraph{Status.}
\textsc{agent-consolidated} (29 August 2026) is the historical checkpoint from
which this revision descends: the definitions and theorems were independently
reconstructed and adversarially checked by an agent, with executable fixtures.
\textsc{normative-continuity-math-settled} (30 August 2026): the structural
mathematical specification, its principal modeling choices, theorem
dependencies, satisfiability, and Lean theorem spine have been settled. This
does not assert Coverage, Progress, substantive normative correctness, Proper
Exercise, or realization by a concrete reasoner. Lean-verified, under their
stated abstract hypotheses: Theorems~\ref{thm:replay}--\ref{thm:nsa},
Lemmas~\ref{lem:matter-continuity}, \ref{lem:route-extinction},
\ref{lem:nonstarve-feasible} and \ref{lem:matter-bookkeeping}, the rotating
countermodel in both directions, and an inhabitant of the issue-trace
specification. Wait responsiveness and non-starvation are assumptions.
