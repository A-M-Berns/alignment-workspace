...
|---|---|---|---|
| **Realized corrective force `beta·g(P)` is the service variable** | **Allocated authority `a = beta`** | `rounds/2026-08-31-normative-affordability/SERVICE_FORCE_TYPING.md` | The realized position is unschedulable (decided at the fixed point), undefined under perfect compliance, and anti-monotone in conformance — it scores a date as better served because the reasoner did worse |
| **Bounded-deficit Surface Fairness gives claim-weighted service fidelity** | **It is a condition on total mass and gives nothing about claim weighting**; only Persistent Relevance interface 1 transfers | `…/SERVICE_TRANSFER.md` §3 | A two-surface fair rotation has service-weighted defect exactly zero at every horizon and claim-weighted defect tending to `1/2` |
| **Contiguity is the service interface primitive** | **A transport plan** (feasibility + stability + service-to-claim cap) | `…/SERVICE_TRANSFER.md` §4 | Contiguity is not checkable at a finite horizon, not quantitative, and false of bounded-delay service, which is normatively unimpeachable |
| **Transport derives contiguity** | **The two routes are incomparable** | `…/SERVICE_TRANSFER.md` §4 | The transport theorem bounds only the density of the *transported* claim measure |
...
| **D1/D3 hold under star-shapedness** | **D1 needs concavity; D2/D3 additionally need equal claim masses** | `…/BOUNDED_DELAY_AFFORDABILITY.md` §2 | With existing load the comparison is between increments (split `2` beats atomic `5/2`); a crossed assignment wins `1.6` vs `6` when a saturating date should take the larger claim |
...
| **`F_r` is a property of the norm, scheduler-independent** | **`F_r` is the misfit landscape read against the chosen service measure** | `…/SERVICEABILITY_FRONTIER.md` §3, `…/JOINT_SERVICEABILITY.md` §1 | A scheduler that services a reason on nearly-satisfied dates gets a small residual; one that services it on deep-exclusion dates does not |
| **The transport residual is the raw sum `sum T eps`** | **the claim-normalized `epsbar_N(T)`** | `…/SERVICE_TRANSFER.md` T3 | The raw sum is extensive and diverges on any infinite claim stream even when every edge is exact to a fixed tolerance |
...
| *realized corrective force* (as the service variable) | **allocated authority** | the term survives for the endogenous position; it is not service |

## 3. Names introduced and still provisional

All of these are **provisional pending a naming audit** and none is settled:
service parsimony; transport stability; overload certificate; fixed-set
contiguity; allocated service / realized force; coercive Actionability; local
authority capacity; friction (the sequence `D_t sqrt(m_t)`); persistence region;
star-shaped date cost; admissible service trace; serviceability frontier; critical
delay; settlement-misfit landscape; Sharp Timely Service; claim-normalized
transport error; nested assessment; deadline insolvency; answerability slice
(inherited from the note); anchored interpretation (inherited).
