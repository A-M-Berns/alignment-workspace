...
## 3. The positive result: faithful acceleration (gated Total Trust → Value)

**Claim.** In the *same* setup, $H$ is forced into **gated (classwise) Total Trust** in $A$, hence **classwise Value** — over its **admissible domain** (all sentences but the quote-referencing 2a family; §4.1), with **no appeal to the prior**.
...
- **(II) Calibration** — exactly the LI theorem **Expectation Unbiasedness From Feedback** applied to the feedback LUV $\ulcorner E^H_{f(n)}(X)\urcorner$. Since $H$ is a fixed computable process (even reading $A$, the coupled recursion is deterministic), $E^H_{f(n)}(X)$ is a definite $\Gamma$-provable number whose realized value enters $A$'s world *by observation* at $\sim f(n)$. The theorem gives, for any $\mathcal{C}_A$-generable divergent patient weighting $w_n$,
  $$\sum_n w_n\big(E^H_{f(n)}(X) - a_n\big) \;=\; o\Big(\textstyle\sum_n w_n\Big).$$
...
Suppose gated Total Trust fails: on a class of infinite weight, $A$ forecasts high ($a_n > t$) while $H$'s own price sits low ($E^H_n(X) < t - \varepsilon$). Form the **doubly-soft, legal** weight $w_n = \mathrm{Ind}_\delta(a_n > t)\cdot \mathrm{Ind}_\delta(t - \varepsilon - E^H_n(X))$ — continuous in the observable $a_n$ and in $H$'s own price. A $\mathcal{C}_H$-trader holds $w_n$ units of $X$ from day $n$ to day $f(n)$ and unwinds. Marked to market it banks
$$
\sum_{n} w_n\Big(\underbrace{E^H_{f(n)}(X) - a_n}_{=\,o(\sum w)\ \text{by (II)}}\Big) + \underbrace{\sum_n w_n(a_n - t)}_{\ge 0} + \varepsilon\sum_n w_n \;\ge\; (\varepsilon - o(1))\sum_n w_n \to \infty,
$$
with bounded risk — an exploit, which $H$ does not admit. So the violation class has bounded weight: **gated Total Trust holds, and classwise Value follows** (via the §1 witness equivalence, summed over the class). *(The §5 trader core of the positive note is kernel-checked, `sorry`-free, in `FaithfulAcceleration.lean`.)*
...
- the **positive result** proves the ceiling is *reached* — the gated/averaged form is forced, and forced maximally (over the whole admissible domain, no prior).

Together they pin the achievable notion of cross-process trust in this setup to a point: **exactly gated/averaged Total Trust → Value, no more and no less.**
...
- The **positive installs the Value rung.** It forces gated Total Trust, and the tight witness carries it — summed over the class — to **gated/classwise Value**. Tightness means **zero loss from Total Trust to Value**: the instrumental target arrives at *exactly* the grade of the epistemic one; everything sacrificed is the tower-surplus Value never needed.
...
| **unfrozen** (coupled $H$) | averaged inequality (gated Total Trust) | all *admissible* sentences | **forced** | positive note (this §3) |
...
- **Positive** ([`faithful-acceleration.md`](faithful-acceleration.md)): the **averaged-inequality** form — gated/classwise Total Trust → Value — *is* forced over its **admissible domain** (all but the quote-referencing 2a family), no prior, from self-trust (I) + calibration (II = Expectation Unbiasedness From Feedback).
