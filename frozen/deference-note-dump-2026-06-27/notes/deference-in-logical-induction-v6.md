...
> - a **second positive construction** — *faithful acceleration*, which forces gated/classwise **Value** by having $A$ predict $H$'s own future credences, over an **admissible domain** rather than only the timely fragment (§5.9);
...
Because the $\beta$-atoms recording $a_n$ live in $H^+$'s own language, "$R_n\le\tfrac12$" is a sentence of that language, decided by $n+1$. Take $P^{(n)}=g_n$ a fresh atom with the axiom $g_n\leftrightarrow(R_n\le\tfrac12)$; then $g_n$ is decided by $F(n)$ to $\mathbb{1}[r_n\le\tfrac12]$, so by provability-induction $Y_n=H^+_{F(n)}(g_n)=\mathbb{1}[r_n\le\tfrac12]+o(1)$, and the Lemma gives
$$
\boxed{\ |a_n-Y_n|\ge\tfrac12-\tfrac1{2n}-o(1)=\tfrac12-o(1)\ }\quad\text{for every }n.
$$
...
- **(II) calibration** = **Expectation Unbiasedness From Feedback** (Garrabrant Thm 4.8.16) at $B_n=\ulcorner Y_n\urcorner$: $H$ is computable (even reading $A$), so $Y_n$ is determined-via-$\Gamma$, and its realized value enters $A$'s world by *observation* at $\sim f(n)$ — hence for any $\mathcal{C}_A$-generable divergent patient $w_n$, $\;\sum_n w_n(Y_n-a_n)=o(\sum_n w_n)$. **(II) is checked after the fact** (wait → observe → bank), so 2b's cost-circularity never arises and **no power assumption** is needed — this is what lets the unfrozen target work where the §4.2 mirror's *timely* target could not.

**Theorem (faithful acceleration — gated/classwise Total Trust, hence classwise Value).** Fix an admissible $X$ and $t,\varepsilon,\delta$; let the violation weight $w_n=\mathrm{Ind}_\delta(a_n>t)\cdot\mathrm{Ind}_\delta(t-\varepsilon-E^H_n(X))$. Then $\sum_n w_n<\infty$: equivalently, averaged over the gated class $\{a_n>t\}$, $H$'s own credence is $\gtrsim t$, and by §1.2 $H$ **Values** $A$ on that class. *Proof (trader):* the **self-trust round-trip** — buy $X$ at $n$, sell at $H$'s own future price $Y_n$, on a sparse schedule with disjoint windows, weighted to fire only where $A$ flags high — banks $\sum_S w_n(Y_n-E^H_n(X)) \ge (\varepsilon-o(1))\sum_S w_n\to\infty$: ingredient (II) kills the forecast-bias term $\sum w_n(Y_n-a_n)$, ingredient (I) makes "sell at $Y_n$" a legitimate cash-out, and one-sidedness supplies the $\varepsilon$. Unbounded value, bounded risk — an exploit $H$ does not admit. The trader core is kernel-checked (`FaithfulAcceleration.lean`; but see §7 — what is checked is the *arithmetic*, with calibration and the criterion named).

**Honest strength.** This is the **gated/averaged** form — the §5.6 / T5 ceiling — *not* the per-day tower (which §4.8 kills off $G$). A day-averaged input (calibration) buys exactly a day-averaged conclusion; the proof never uses pointwise tracking $a_n\approx Y_n$, which is ruled out for the real human. The honest match: averaged in, averaged out. (On $G$ the frozen construction *does* get the per-day tower; the two are complementary — §5.11.)
...
The faithful-acceleration Value (§5.9) is **gated/averaged**, and two limits of that form belong in the headline, not a footnote (both surfaced by the `deference-trust-lab` red-team):
...
- Claude Opus 4.8 (with Demski), **"Faithful Acceleration"** — `faithful-acceleration.md`. The §5.9 credence round-trip and gated/classwise Value. Trader core kernel-checked in `lean-deference/FaithfulAcceleration.lean`. *Its "all sentences" scope is corrected here to the admissible domain (§5.10).*
...
| claim | status | Δ | where |
|---|---|---|---|
| gated/classwise Total Trust → Value (credence round-trip) | C (arithmetic chain); calibration `hbias` + criterion `hbdd` **named** | **+** | §5.9 |
