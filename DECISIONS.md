...
its wiki page `Continuation-BRIA`.  (1) A continuation hypothesis is `h(χ_k) = (c_{h,k},
e_{h,k})`: a causal advisor continuation for the block and an accountable contextual
claim, at the **system's** block contract `χ_k = (H_k, m_k, Exec^G, R_k)`; the earlier
`(q, τ, e)` form with a "treatment" component is demoted to an explanatory
factorization `c = τ(q)`.  (2) A test is the derived event that the selected
continuation is executed under the block contract; data is evidence about a claim only
when generated by executing the continuation the claim names.  (3) The block duration
`m_k` is system-scheduled and revealed at the block's opening; bidders do not choose
horizons (hypothesis-requested durations OPEN).  (4) The weighted auction credits the
round's subsidy at the opening of the round it funds — the paper's auction under a
reindexed allowance with an initial endowment, a named modification; capital adequacy is
`A_i(K) − w_K → ∞` with the subsidy through `K` inclusive (the timing audit's interim
factor 2 removed by the cancellation of the claim at a wealth-constrained win, Lean
`record_succ_lt_of_rejected_opening`); the prefix rule is `A(k,i) = ΔM_k + 1/k` on
`s(k) = ⌊√(S_k/M_k)⌋`.  (5) Non-dominance `m_K/S_K → 0` is the sharp schedule-level
boundary: the prefix-online opening-timed auction covers every c.e. e.c. class exactly
there, no estimating agent satisfies the criterion beyond it, and under the paper's
settlement timing no prefix-online rule serves every non-dominant schedule.  (6)
`Regret = LEARN + SLACK + SHIFT` are three separate interfaces: continuation BRIA
