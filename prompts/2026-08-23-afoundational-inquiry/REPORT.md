...
1. Grounds, normative license, and account incidence are independent certificate
   projections. Empirical grounds cannot fill a normative-license edge.
2. Strictly earlier license parents reject self-license and same-transition
...
10. A fixed docket with normalized monotone submodular progress and metric
    context costs is MLSC; unit metric is submodular ranking. Complementarity is
    an explicit counterexample.
11. Adaptive-submodular and interactive-submodular schedulers are special classes
    with fixed uncertainty, response, and objective semantics. Mutable service
    rules require an accounted instance transition.
12. External evaluator-relative due status can disagree with internal `Must`
    generation. The former remains a benchmark, not the positive definition.
13. Actual-run genealogy and answerability do not establish non-capture. `R -> O`
    remains open; downstream enforcement is unchanged.

## Deviations

No Lean file or claim registry entry was created because the candidate types are
not frozen and the results are interface derivations plus finite witnesses. The
round uses `REPORT.md`, required by `AGENTS.md`, in addition to the prompt's
suggested files. The status map is in `THEOREM_MAP.md`; no new structured theorem
interface was added because the live emitter supports one theorem-facing
interface and this round does not replace it. No broad rewrite of the existing
legitimacy README was made.
...
`afoundational inquiry interface`, `authority genealogy`, `due token`, `coverage
debt`, `event-time accrual`, `docket coverage`, `service coverage`, and `service
specification integrity`.

## Outstanding maintainer actions

None required for this research-round PR. Naming and formal promotion remain
future decisions rather than merge blockers.
