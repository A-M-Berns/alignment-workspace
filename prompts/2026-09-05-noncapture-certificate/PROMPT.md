...
# Your lane: Agent C — Non-Capture certificate minimization

Round name: `2026-09-05-noncapture-certificate`. Branch: `round/2026-09-05-noncapture-certificate`.

You are an interface-minimization worker. You are NOT to invent a universal counterfactual
semantics.

Problem: **determine the weakest external certificate the legitimacy theory actually needs
in order to derive Robust Openness.**

Treat declared normative scope `Γ` and declared intervention class `𝓙` as application
inputs. Start from the coverage contract of the cf-coverage round
(`σ = (a, Γ, Rel, Adeq, Disp)`, `Active`, `Implements`, the self-sealing safety theorem
and its local-closure-adequacy hypothesis) and the interaction interface
`(Ω_h, Q_h, Z_h, β_h)`, and from the principal-relative standing form of the defeat
rounds. Start schematically from `NonCaptureCert(P, Γ, 𝓙, 𝔍)` where `P` is a protected
principal or declared relation and `𝔍` the external intervention semantics.

Determine which properties must survive interventions in `𝓙`. Candidates: target /
concern identity; applicability; representability; route existence; standing / access;
route efficacy; ability to produce an accountable receipt; relevant settlement /
evaluation access. **Do not include a clause because it sounds normatively desirable.**
For each clause give an attack — a concrete history plus intervention in which Robust
Openness fails while every other clause holds — or remove it. Exact finite fixtures in
`src/` + `tests/run.py` for each attack; Lean only if a clause's necessity is naturally a
small structural statement.

Separate: actual-prefix Coverage; counterfactual persistence of the route; counterfactual
persistence of the concern's identity/applicability; counterfactual efficacy once the
route is exercised. Ask exactly what coupling / holding-fixed information an external
counterfactual framework must provide (what is held fixed — the exterior response policy?
the settlement engine? the principal's policy? — and what may vary).

Compare at least three candidate external intervention semantics — e.g. the DR-MDP
excision-of-ancestry criterion already in the repository, an interventional/do-style
coupling on the interaction interface, and a policy-class quantification like the
deference line's "under every advisor policy in scope" — ONLY to determine the bill our
theory issues, i.e. the common outputs each would have to deliver. Do not choose one.

Test the typing of the current wiki phrase that settlement "cannot be delayed or
suppressed": is that an Integrity/interface property or a Non-Capture property? Argue with
an example.

Desired output: an interface theorem of the form
`Coverage_actual + NonCaptureCert ⇒ RobustOpen`, stated exactly, with a paper proof and
an exact fixture instantiating every hypothesis (nonvacuity) and one fixture per dropped
clause showing the conclusion fails without it.

Deliverables: `CERTIFICATE.md` (the minimal schema, each clause with its necessity
attack), `SEMANTICS_COMPARISON.md` (what each external framework must supply; what stays
outside), `INTEGRITY_BOUNDARY.md` (exact distinction from Integrity and from settlement
trust, with the delay/suppression typing), `THEOREMS.md`, fixtures, `REPORT.md`. Say what
an application must instantiate and what remains genuinely outside the generic theory.
