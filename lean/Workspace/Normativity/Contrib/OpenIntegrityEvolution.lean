/-
# Accounted obligation states, Integrity evolution, and the open Integrity evolution
...
occurrence's load) are available to it.  `OpenIntegritySegment` requires
...
namespace Workspace.Normativity.Contrib.OpenIntegrityEvolution
...
/-! ## 4. Openness over time and the open Integrity evolution -/
...
/-- **The open Integrity evolution**, as a certificate: an Integrity evolution every
state of which is robustly open.  It is the conjunction of the Integrity component of
internal legitimacy with the Robust Openness component of external legitimacy, stated on
the accounted record alone; the legitimacy of a segment (`Deference.Contrib.Legitimacy`)
adds authorship and transparency.  Proof-relevant; `OpenIntegrity` below hides it. -/
structure OpenIntegritySegment (sem : OpennessSemantics S anchor Γ J R)
    (O₀ O₁ : ObligationState S anchor) where
  evolution : Evolution S anchor O₀ O₁
  openAll : evolution.AllStates (OpenAt sem)

namespace OpenIntegritySegment

variable {S anchor} {sem : OpennessSemantics S anchor Γ J R}

def refl (O : ObligationState S anchor) (h : OpenAt sem O) :
    OpenIntegritySegment S anchor sem O O :=
  ⟨.refl O, h⟩

/-- Composition at a shared state. -/
def trans {O₀ O₁ O₂ : ObligationState S anchor}
    (left : OpenIntegritySegment S anchor sem O₀ O₁)
    (right : OpenIntegritySegment S anchor sem O₁ O₂) :
    OpenIntegritySegment S anchor sem O₀ O₂ :=
...
theorem answerable {O₀ O₁ : ObligationState S anchor}
    (seg : OpenIntegritySegment S anchor sem O₀ O₁) :
    Conservation S anchor O₀ O₁ ∧ OpenAt sem O₀ ∧ OpenAt sem O₁ :=
  ⟨seg.evolution.conservation, seg.openAll.head, seg.openAll.last⟩

end OpenIntegritySegment

/-- The endpoint relation a consumer may use; the witness stays available for audit. -/
def OpenIntegrity (sem : OpennessSemantics S anchor Γ J R) (O₀ O₁ : ObligationState S anchor) :
    Prop :=
  Nonempty (OpenIntegritySegment S anchor sem O₀ O₁)

theorem OpenIntegrity.trans {sem : OpennessSemantics S anchor Γ J R}
    {O₀ O₁ O₂ : ObligationState S anchor} (h₁ : OpenIntegrity S anchor sem O₀ O₁)
    (h₂ : OpenIntegrity S anchor sem O₁ O₂) : OpenIntegrity S anchor sem O₀ O₂ :=
...
open at every prefix the two open Integrity segments compose, and the composite's
accounts are `answered` and `live`.  Under a semantics closed only at the middle prefix,
both endpoints are open and the evolution is not an open Integrity evolution. -/
...
def seg₀₁ : OpenIntegritySegment protocol wAnchor semOpen state₀ state₁ :=
...
open under `semMiddle`, the middle state is not, and the evolution is not an open
Integrity evolution. -/
...
end Workspace.Normativity.Contrib.OpenIntegrityEvolution

#print axioms Workspace.Normativity.Contrib.OpenIntegrityEvolution.Evolution.propagate_toSegment
#print axioms Workspace.Normativity.Contrib.OpenIntegrityEvolution.Evolution.ofSegment
#print axioms Workspace.Normativity.Contrib.OpenIntegrityEvolution.Evolution.AllStates.trans
#print axioms Workspace.Normativity.Contrib.OpenIntegrityEvolution.Conservation.trans
#print axioms Workspace.Normativity.Contrib.OpenIntegrityEvolution.Conservation.ofStep
#print axioms Workspace.Normativity.Contrib.OpenIntegrityEvolution.Evolution.conservation
#print axioms Workspace.Normativity.Contrib.OpenIntegrityEvolution.OpenIntegritySegment.trans
#print axioms Workspace.Normativity.Contrib.OpenIntegrityEvolution.OpenIntegritySegment.answerable
#print axioms Workspace.Normativity.Contrib.OpenIntegrityEvolution.OpenIntegrity.trans
#print axioms Workspace.Normativity.Contrib.OpenIntegrityEvolution.Witness.composed
#print axioms Workspace.Normativity.Contrib.OpenIntegrityEvolution.Witness.endpoint_only_insufficient
