...
| `tests/lean_scope.py`; the `lean` job's scope condition in `.github/workflows/ci.yml` | Claude Opus 5 (Anthropic) | `ci-only` | 2026-08-17 | `prompts/2026-08-17-ci-and-friction/` — a trust-chain edit on direct maintainer instruction, answering `PRIORITIES.md` item 10. The gate still runs and still reports; only the work is conditional, and the decision fails closed |
| `tests/round_records.py`; the ledger's append convention in `DECISIONS.md`; the generated views moved to `state/views/` and `checkers/workspace_state.py`'s render target | Claude Opus 5 (Anthropic) | `ci-only` | 2026-08-17 | `prompts/2026-08-17-ci-and-friction/` — the maintainer's friction rulings, taken in conversation. A trust-chain change: one new gate, and a modification of an existing house checker |
